...
def make_combined_features(dataframe):
    fingerprints = dataframe_to_morgan(dataframe)
    ...
    return np.hstack(
        [
            fingerprints.astype(np.float32),
            descriptors,
        ]
    )
...
def main():
    ...
    X_train = make_combined_features(train)
    X_validation = make_combined_features(validation)
    X_test = make_combined_features(test)
    ...
    print("=== Morgan + Descriptor RF Scaffold Validation ===")
    for key, value in validation_metrics.items():
        print(f"{key}: {value}")

    print()
    print("=== Morgan + Descriptor RF Scaffold Test ===")
